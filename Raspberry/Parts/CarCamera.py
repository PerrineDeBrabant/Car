...
class CarCamera(object):
	def __init__(self,_params):
		...
		#The camera needs to warm up
		time.sleep(1)
	#When you are in train mode you can save the pictures to a file called train_photos
	def save_frame(self,_turn):
		print("save my frames")
		name = str(Utils.ms_epoch())+":"+str(_turn)
		print("The picture will be saved")
		print(self.path+"/train_photos/"+name+".jpg")
...
class ObjectDetection(object):
	def __init__(self):
		self.stop_detected = False
		self.classifier = cv2.CascadeClassifier('static/config/stop_sign.xml')
